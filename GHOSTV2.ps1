...
.DESCRIPTION
    A native PowerShell daemon that spoofs AMD RDNA hardware as NVIDIA RTX GPUs,
    injects the ZLUDA translation layer, and provides a Waiting Room TUI with DOOM.
#>

$ErrorActionPreference = "SilentlyContinue"
$GhostDir = "$env:USERPROFILE\.ghost"
$ZludaDir = "$GhostDir\zluda"
$DoomDir = "$GhostDir\doom"
$MusicDir = "$GhostDir\music"
$ConfigDir = "$env:USERPROFILE\.config"
$ConfigFile = "$ConfigDir\ghost_env_setup.ps1"

# ANTI-NESTING LOCK: Prevent recursive shell spawning
...
# ==============================================================================
# PHASE 1: HARDWARE DETECTION & JSON MAPPING
# ==============================================================================

function Get-AmdGpuSeries {
    # MULTI-GPU FILTERING: Sort by AdapterRAM descending to prioritize dGPU over iGPU
    $gpu = Get-CimInstance Win32_VideoController | Where-Object { $_.Name -match "AMD|Radeon|Advanced Micro Devices" } | Sort-Object -Property AdapterRAM -Descending | Select-Object -First 1
    if (-not $gpu) { return "UNKNOWN" }

    if ($gpu.Name -match "(?:RX|Pro W)\s*(9|8|7|6|5)\d{3}\b") {
...
function Set-GhostEnvironment {
    $series = Get-AmdGpuSeries
    
    $scriptDir = if ($PSScriptRoot) { $PSScriptRoot } else { Get-Location }
    $parentDir = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($scriptDir, ".."))
    
    # AGGRESSIVE MAPPING.JSON SEARCH (Fixed for 'configs' repo structure)
    $searchPaths = @(
    ...
    [Environment]::SetEnvironmentVariable("HIP_VISIBLE_DEVICES", "0", "Process")
    ...
    Write-Host "[GHOST] Environment Active. GPU Spoofed as $spoof_name. GFX Masked to $gfx_mask." -ForegroundColor Green
}
...
function Toggle-Music {
    ...
    if ($global:MediaPlayer.playState -eq 3) {
        ...
        if (-not (Test-Path $track) -or (Get-Item $track).Length -lt 100000) {
            ...
            Invoke-WebRequest -Uri "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-4.mp3" -OutFile $track
        }
...
function Launch-Doom {
    ...
    if (-not $doomExe) {
        ...
        New-Item -ItemType Directory -Force -Path $DoomDir | Out-Null
        
        $zipPath = "$env:TEMP\doom_$PID.zip"
        Invoke-WebRequest -Uri "https://github.com/chocolate-doom/chocolate-doom/releases/download/chocolate-doom-3.0.1/chocolate-doom-3.0.1-win32.zip" -OutFile $zipPath
        Expand-Archive -Path $zipPath -DestinationPath $DoomDir -Force
        Remove-Item $zipPath -ErrorAction SilentlyContinue

        $extractedExe = Get-ChildItem -Path $DoomDir -Filter "chocolate-doom.exe" -Recurse | Select-Object -First 1
        if ($extractedExe) {
            Move-Item -Path "$($extractedExe.DirectoryName)\*" -Destination $DoomDir -Force
            Remove-Item -Recurse -Force $extractedExe.DirectoryName
        }

        Write-Host "[GHOST] Downloading DOOM Shareware WAD..." -ForegroundColor Cyan
        Invoke-WebRequest -Uri "https://distro.ibiblio.org/slitaz/sources/packages/d/doom1.wad" -OutFile "$DoomDir\DOOM1.WAD"
        
        $doomExe = Get-ChildItem -Path $DoomDir -Filter "chocolate-doom.exe" -Recurse | Select-Object -First 1 | Select-Object -ExpandProperty FullName
    }
...
function Get-TrueVRAM {
    ...
    if (Test-Path $cacheFile) {
        ...
        if ($fileInfo.LastWriteTime -gt (Get-Date).AddHours(-24)) {
            return Get-Content $cacheFile
        }
    }

    $dxdiagFile = "$env:TEMP\dxdiag_ghost_$PID.xml"
    
    # DXDIAG RACE CONDITION FIX: Added -Wait to ensure the XML is fully written before parsing
    if (-not (Get-Process -Name "dxdiag" -ErrorAction SilentlyContinue)) {
        Start-Process -FilePath "dxdiag" -ArgumentList "/x `"$dxdiagFile`"" -WindowStyle Hidden -Wait
    }
    
    if (Test-Path $dxdiagFile) {
        try {
            $fileInfo = Get-Item $dxdiagFile
            if ($fileInfo.Length -gt 0) {
                $fs = [System.IO.File]::Open($dxdiagFile,[System.IO.FileMode]::Open, [System.IO.FileAccess]::Read,[System.IO.FileShare]::ReadWrite)
                $reader = New-Object System.IO.StreamReader($fs)
                $xmlString = $reader.ReadToEnd()
                $reader.Close()
                $fs.Close()
                
                if ($xmlString -match "</DxDiag>") {
                    try {
                        [xml]$dxdiag = $xmlString
                        # MULTI-GPU FILTERING: Sort by DedicatedMemory descending to prioritize dGPU over iGPU
                        $amdDevices = @($dxdiag.DxDiag.DisplayDevices.DisplayDevice | Where-Object { $_.CardName -match "AMD|Radeon|Advanced Micro Devices" })
                        $amdDevice = $amdDevices | Sort-Object { [int]($_.DedicatedMemory -replace '\D','') } -Descending | Select-Object -First 1
                        
                        if ($amdDevice) {
                            $vramMB = $amdDevice.DedicatedMemory -replace '\D',''
                            $vramGB = [math]::Round([int]$vramMB / 1024, 1).ToString() + " GB"
                            
                            if (-not (Test-Path $ConfigDir)) { New-Item -ItemType Directory -Force -Path $ConfigDir | Out-Null }
                            $vramGB | Out-File -FilePath $cacheFile -Encoding utf8
...
function Show-WaitingRoom {
    ...
    # TUI FLICKER REMOVAL: Hide the blinking cursor to make it look like a native app
    ...
    while ($true) {
        ...
        if ($foundPort -or $logReady) {
            ...
            if ($global:DoomProcess -and -not $global:DoomProcess.HasExited) {
                Stop-Process -Id $global:DoomProcess.Id -Force
            }
        ...
        Write-Host "╔══════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
        Write-Host "║                 " -NoNewline -ForegroundColor Cyan; Write-Host "GHOST ENVIRONMENT (v2.0)" -NoNewline -ForegroundColor Green; Write-Host "                 ║" -ForegroundColor Cyan
        Write-Host "╠══════════════════════════════════════════════════════════╣" -ForegroundColor Cyan
        Write-Host ("║  {0,-56}║" -f $status) -ForegroundColor Cyan
        Write-Host "║                                                          ║" -ForegroundColor Cyan
        Write-Host ("║  {0,-56}║" -f "[ GPU: Spoofed as $spoof ]  [ GFX MASK: Active ]") -ForegroundColor Cyan
        Write-Host ("║  {0,-56}║" -f "[ TEMP: $temp ][ VRAM: $trueVram ]") -ForegroundColor Cyan
        if ($ModeFlag -eq "ZLUDA") {
            Write-Host "║  " -NoNewline -ForegroundColor Cyan; Write-Host ("{0,-56}" -f $backend) -NoNewline -ForegroundColor Yellow; Write-Host "║" -ForegroundColor Cyan
        } else {
            Write-Host ("║  {0,-56}║" -f $backend) -ForegroundColor Cyan
        }
        Write-Host "╠══════════════════════════════════════════════════════════╣" -ForegroundColor Cyan
        Write-Host "║  HOTKEYS:                                                ║" -ForegroundColor Cyan
        Write-Host ("║  {0,-56}║" -f "[D] Play DOOM          [M] Toggle Music") -ForegroundColor Cyan
        Write-Host "╚══════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    ...
    # Safety restore cursor if loop breaks unexpectedly
    [Console]::CursorVisible = $true
}

# PROCESS TREE SLAUGHTER FIX: Nuclear fallback for elevated zombie processes
...
function ghost-start {
    ...
    if ($args.Length -gt 1) {
        foreach ($arg in $args[1..($args.Length-1)]) {
            if ($arg -match "\s") { $argArray += "`"$arg`"" } else { $argArray += $arg }
        }
    }
    ...
    try {
        ...
        if ($process.HasExited) {
            ...
            if ($process.ExitCode -ne 0 -or $errContent -match "No HIP backend|CUDA out of memory|Torch not compiled with ROCm" -or $outContent -match "No HIP backend|CUDA out of memory|Torch not compiled with ROCm") {
                $needsFailover = $true
            }
        }
        ...
        if ($needsFailover) {
            ...
            if (Test-Path "$ZludaDir\zluda.exe") {
                # CLEANUP ERROR SUPPRESSION: Give OS time to release file handles
        ...
        Remove-Item "$env:TEMP\dxdiag_ghost_$PID.xml" -ErrorAction SilentlyContinue
        Remove-Item $logOut, $logErr -ErrorAction SilentlyContinue
    }
}
...
if ($env:GHOST_ENV_ACTIVE -ne "1") {
    ...
    [Environment]::SetEnvironmentVariable("GHOST_ENV_ACTIVE", "1", "Process")
    Start-Process powershell.exe -ArgumentList "-NoExit", "-ExecutionPolicy Bypass", "-File", "`"$scriptPath`"" -NoNewWindow
    exit
} else {
